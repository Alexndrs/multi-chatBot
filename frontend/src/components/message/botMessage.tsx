...
export function BotMessage({ splittedMessage, isMulti, isLast, setError }: { splittedMessage: splittedMessage, isMulti?: boolean, isLast?: boolean, setError?: (error: string | null) => void }) {

    const [showThinking, setShowThinking] = useState(false);
    const { regenerateMessage, chooseReply } = useConversationLogic();

    const msgIsNotEmpty = splittedMessage.mainContent.trim() !== '' || splittedMessage.thinkContent.trim() !== '';

    const handleChooseReply = async () => {
        try {
            await chooseReply(splittedMessage.convId, splittedMessage.msgId);
        } catch (error) {
            console.log("should show modal with error : ", error);
    ...
    return (
        <div key={splittedMessage.msgId} className={`flex flex-col py-4 gap-2 ${isMulti ? 'bg-white/1 rounded-lg px-8' : 'px-4 md:px-40'}`}>
            ...
            <div className={'pb-2 whitespace-pre-wrap flex-1}'}>
                ...
                <div className={`hide-scrollbar overflow-y-auto ${isMulti ? 'max-h-[50vh]' : ''}`}>
                    <ReactMarkdown
            ...
            {isLast && isMulti && msgIsNotEmpty && (
                <button className="bg-blue-500 text-white px-4 py-2 rounded-lg shadow hover:bg-blue-600 cursor-pointer transition-colors duration-200 w-fit mx-auto text-md" onClick={handleChooseReply}>
